...
_ROOT    = Path(__file__).parent.parent            # 퇴직연금 _XAI/
DATA_DIR  = _ROOT / 'data'
OUT_DIR   = _ROOT / 'results' / 'step4'
...
# 5. 저장
# ═══════════════════════════════════════════════════════
weights_df.to_parquet(OUT_DIR / 'portfolio_weights.parquet')
perf_df.drop(columns='year').to_parquet(OUT_DIR / 'portfolio_performance.parquet')

annual.to_csv(OUT_DIR / 'summary.csv', encoding='utf-8-sig')

# 비중 피벗 (날짜 × 슬롯, CSV)
weights_pct = (weights_df * 100).round(1)
weights_pct.index = weights_pct.index.strftime('%Y-%m-%d')
weights_pct.to_csv(OUT_DIR / 'weights_pivot.csv', encoding='utf-8-sig')
...
print(f"[저장] summary.csv                    → {OUT_DIR}")
print(f"[저장] weights_pivot.csv              → {OUT_DIR}")
