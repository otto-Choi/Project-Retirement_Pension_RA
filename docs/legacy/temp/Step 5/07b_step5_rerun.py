...
변경 내용 (기존 07_step5_constrained.py 대비):
  ...
  6. 데이터 경로를 실제 notebooks/ 디렉터리로 수정

입력:
  notebooks/입력_데이터/index_returns.parquet
  notebooks/입력_데이터/slot_returns.parquet
  notebooks/입력_데이터/year_end_best.csv
  notebooks/입력_데이터/mar_series.parquet

출력 (notebooks/step5_지역제약포트폴리오_최종/):
  ...
  step5_comparison.csv                    (덮어쓰기)
  step5_weights_constrained.csv           (덮어쓰기)
"""
...
# 경로 설정
# ─────────────────────────────────────────────
ROOT     = Path(__file__).parent.parent.parent          # 퇴직연금 _XAI/
NB_DIR   = ROOT / 'notebooks'
DATA_DIR = NB_DIR / '입력_데이터'
OUT_DIR  = NB_DIR / 'step5_지역제약포트폴리오_최종'
...
print(f"  step5_comparison.csv")
print(f"  step5_weights_constrained.csv")
