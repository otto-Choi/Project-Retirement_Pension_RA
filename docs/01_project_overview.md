...
### 즉시 착수 가능 (portfolio_engine.py 확장)

1. **CVaR 모듈**: `get_cvar(w_risky, query_date)` — historical simulation 95%/99%
2. **CAL 동적 테이블**: `get_cal_curve(query_date)` — w_risky 0~70% 구간 연속 산출
3. **스트레스 테스트**: `get_stress_test(scenario)` — 2020 COVID, 2022 금리충격 구간 성과

### 병렬 착수 가능

4. **기대수익률 표시 명확화**: BL Π(모델 내부용) vs 역사적 롤링 평균(사용자 표시용) 분리
5. **즉석 리밸런싱**: `get_portfolio(query_date, realtime=True)` — 임의 시점 Σ_down·Π 즉석 재산출
6. **금액 환산**: `current_balance` 시 슬롯별 `amount_krw` 필드 + "X만원" 문자열 반환

### 텍스트 파이프라인 (페르소나 팀 영역)

7. **텍스트 파이프라인 코드 실행**: Embedding → KMeans → GPT 점수화 → z-score
8. **Risk Score 계산 코드**: 6개 변수 가중합 + w_risky 조회

### 마지막

9. **전체 UI 텍스트**: "추천" → "참고 정보" 표현 일괄 교체
10. **Streamlit 데모**: 4개 화면 프로토타입
